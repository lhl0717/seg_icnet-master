...
iou_thresholds = [x / 100 for x in range(40, 100, 5)]
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        #print(np_masks.astype(bool))
        # 判断是否同一行
        def same_row(point, last_point):
            return point[0] == last_point[0]

        # 遍历mask集合
        for mask_i in range(np_masks.shape[0]):
            np_mask = np_masks[mask_i]
            # 转换为与图像分辨率大小一致的矩阵
            np_mask = np_mask.reshape(np_mask.shape[0], np_mask.shape[1])
            # 边缘检测
            counter = np.array(find_contours(np_mask, 0.5)[0], dtype=np.int)
            # 去除相同元素
            counter = np.unique(counter, axis=0)
            l_line = []
            r_line = []
            l_line.append(counter[0])
            # 遍历边缘点集合,输出左右两条斜边
            for point_i in range(1, len(counter)):
                cur_point = counter[point_i]
                last_point = counter[point_i - 1]
                # 比较当前点和上一点是否在同一行，
                # 若换行则把当前点插入l_line，把上一点插入r_line
                if not same_row(cur_point, last_point):
                    r_line.append(last_point)
                    l_line.append(cur_point)
            # 导入拟合左右边函数
            from poly_line import polyLR
            # 返回下倾角，插入列表
            deg_list.append(polyLR(l_line, r_line))
            mask_aera = np_mask.size
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                print(_class)
                text_str = '%s: %.2f  degree: %.2f' % (_class+' '+str(j), score, deg_list[j]) if args.display_scores else _class
...
def evalimage(net: Yolact, path: str, save_path: str = None):
    ...
    preds = net(batch)
    # default_save_path = '/media/tyy/learning/Mask_RCNN-master/samples/antenna/dataset/yolact_result/2.png'


     ####  proto 模型生成 ####

    #print(preds[0]['detection'].keys())
    ...
    if save_path is None:
        ...
        plt.title(path)
        # plt.savefig(default_save_path, format='png', transparent=True, dpi=300, pad_inches=0)
        plt.show()
    else:
